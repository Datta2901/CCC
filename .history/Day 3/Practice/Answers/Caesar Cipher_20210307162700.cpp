...
long minArea(vector<int> Xco, vector<int> Ycoordinate, int k) {
       
    sort(Xco.begin(),Xco.end());
    sort(Ycoordinate.begin(),Ycoordinate.end());
    
    long length = Xco.size();
    long differenceX = Xco[k-1] - Xco[0];
    long differenceY = Ycoordinate[k-1] - Ycoordinate[0];
    
    for (long i=1; i<length-k+1; ++i){
        differenceX = differenceX<(Xco[i+k-1]-Xco[i])? differenceX : Xco[i+k-1]-Xco[i];
    }

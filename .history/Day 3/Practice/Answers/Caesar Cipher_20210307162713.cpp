...
l minArea(vector<int> Xcoordinate, vector<int> Ycoordinate, int k) {
       
    sort(Xcoordinate.begin(),Xcoordinate.end());
    sort(Ycoordinate.begin(),Ycoordinate.end());
    
    l length = Xcoordinate.size();
    l differenceX = Xcoordinate[k-1] - Xcoordinate[0];
    l differenceY = Ycoordinate[k-1] - Ycoordinate[0];
    
    for (l i=1; i<length-k+1; ++i){
    ...
    l temp = max(differenceX,differenceY);
    if((temp+2)*(temp+2)==25)
    return 36;
    else{

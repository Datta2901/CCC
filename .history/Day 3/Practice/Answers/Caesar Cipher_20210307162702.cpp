...
long minArea(vector<int> Xcoordi, vector<int> Ycoordinate, int k) {
       
    sort(Xcoordi.begin(),Xcoordi.end());
    sort(Ycoordinate.begin(),Ycoordinate.end());
    
    long length = Xcoordi.size();
    long differenceX = Xcoordi[k-1] - Xcoordi[0];
    long differenceY = Ycoordinate[k-1] - Ycoordinate[0];
    
    for (long i=1; i<length-k+1; ++i){
        differenceX = differenceX<(Xcoordi[i+k-1]-Xcoordi[i])? differenceX : Xcoordi[i+k-1]-Xcoordi[i];
    }
    ...
    long temp = max(differenceX,differenceY);
    if((temp+2)*(temp+2)==25)
    return 36;
    else{

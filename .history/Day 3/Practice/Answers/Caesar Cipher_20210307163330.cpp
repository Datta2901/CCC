...
// //         if (wo[i] >= 'matrix' && wo[i] <= 'z'){  
// //             wo[i] = ((wo[i] - 'matrix' + k) % 26) + 'matrix';       
// //         }
// //         else if (wo[i] >= 'A' && wo[i] <= 'Z'){  
// //             wo[i] = ((wo[i] - 'A' + k) % 26) + 'A';
// //         }
// //     }
// //     cout << wo << endl;
...
//  * The function accepts STRING wo as parameter.
//  */
// int getMaxDeletions(string wo) {
//     int up=0,down=0,left=0,right=0,answer=0;
//     for(int i=0;wo[i]!='\0';i++){
//         if(wo[i]=='U')
//             up++;
//         if(wo[i]=='D')
//             down++;
...
using namespace std;
   ...
   string GetGoodBinaryString(string wo) {
      string answer = "";
      vector<string> temp;
      int i = 0;
      for (int j = 0, count = 0; j < wo.size(); j++) {
         if (wo[j] == '1') {
            count++;
         }
         else
         count--;
         if (count == 0) {
            temp.push_back("1" + GetGoodBinaryString(wo.substr(i + 1,
            j - i - 1)) + "0");
            i = j + 1;
         }
      }

...
long minArea(vector<int> x, vector<int> Ycoor, int k) {
    ...
    long differenceY = Ycoor[k-1] - Ycoor[0];
    ...
    for (long i=1; i<length-k+1; ++i)
    {
        differenceY = differenceY<(Ycoor[i+k-1]-Ycoor[i])? differenceY : Ycoor[i+k-1]-Ycoor[i];
    }

...
using namespace std;
   ...
   string GetGoodBinaryString(string ) {
      string answer = "";
      vector<string> temp;
      int i = 0;
      for (int j = 0, count = 0; j < .size(); j++) {
         ...
         count--;
         if (count == 0) {
            temp.push_back("1" + GetGoodBinaryString(.substr(i + 1,
            j - i - 1)) + "0");
            i = j + 1;
         }
      }

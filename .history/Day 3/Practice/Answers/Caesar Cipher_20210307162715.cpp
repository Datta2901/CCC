...
long long  minArea(vector<int> Xcoordinate, vector<int> Ycoordinate, int k) {
       
    sort(Xcoordinate.begin(),Xcoordinate.end());
    sort(Ycoordinate.begin(),Ycoordinate.end());
    
    long long  length = Xcoordinate.size();
    long long  differenceX = Xcoordinate[k-1] - Xcoordinate[0];
    long long  differenceY = Ycoordinate[k-1] - Ycoordinate[0];
    
    for (long long  i=1; i<length-k+1; ++i){
    ...
    long long  temp = max(differenceX,differenceY);
    if((temp+2)*(temp+2)==25)
    return 36;
    else{

...
long minArea(vector<int> Xcoor, vector<int> Ycoordinate, int k) {
       
    sort(Xcoor.begin(),Xcoor.end());
    sort(Ycoordinate.begin(),Ycoordinate.end());
    
    long length = Xcoor.size();
    long differenceX = Xcoor[k-1] - Xcoor[0];
    long differenceY = Ycoordinate[k-1] - Ycoordinate[0];
    
    for (long i=1; i<length-k+1; ++i){
        differenceX = differenceX<(Xcoor[i+k-1]-Xcoor[i])? differenceX : Xcoor[i+k-1]-Xcoor[i];
    }

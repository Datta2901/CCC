...
long minArea(vector<int> x, vector<int> Ycoord, int k) {
    ...
    long differenceY = Ycoord[k-1] - Ycoord[0];
    ...
    for (long i=1; i<length-k+1; ++i)
    {
        differenceY = differenceY<(Ycoord[i+k-1]-Ycoord[i])? differenceY : Ycoord[i+k-1]-Ycoord[i];
    }

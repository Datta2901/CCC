...
long i minArea(vector<int> Xcoordinate, vector<int> Ycoordinate, int k) {
       
    sort(Xcoordinate.begin(),Xcoordinate.end());
    sort(Ycoordinate.begin(),Ycoordinate.end());
    
    long i length = Xcoordinate.size();
    long i differenceX = Xcoordinate[k-1] - Xcoordinate[0];
    long i differenceY = Ycoordinate[k-1] - Ycoordinate[0];
    
    for (long i i=1; i<length-k+1; ++i){
    ...
    long i temp = max(differenceX,differenceY);
    if((temp+2)*(temp+2)==25)
    return 36;
    else{

...
long minArea(vector<int> x, vector<int> Ycoo, int k) {
       
    sort(x.begin(),x.end());
    sort(Ycoo.begin(),Ycoo.end());
    
    long length = x.size();
    long differenceX = x[k-1] - x[0];
    long differenceY = Ycoo[k-1] - Ycoo[0];
    
    for (long i=1; i<length-k+1; ++i){
        differenceX = differenceX<(x[i+k-1]-x[i])? differenceX : x[i+k-1]-x[i];
    }
    
    for (long i=1; i<length-k+1; ++i)
    {
        differenceY = differenceY<(Ycoo[i+k-1]-Ycoo[i])? differenceY : Ycoo[i+k-1]-Ycoo[i];
    }

...
long lo minArea(vector<int> Xcoordinate, vector<int> Ycoordinate, int k) {
       
    sort(Xcoordinate.begin(),Xcoordinate.end());
    sort(Ycoordinate.begin(),Ycoordinate.end());
    
    long lo length = Xcoordinate.size();
    long lo differenceX = Xcoordinate[k-1] - Xcoordinate[0];
    long lo differenceY = Ycoordinate[k-1] - Ycoordinate[0];
    
    for (long lo i=1; i<length-k+1; ++i){
    ...
    long lo temp = max(differenceX,differenceY);
    if((temp+2)*(temp+2)==25)
    return 36;
    else{
